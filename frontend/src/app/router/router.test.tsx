...
test('mantém filtros e permite entrada direta em uma ficha', async () => {
  const router = createAppRouter(createMemoryHistory({ initialEntries: ['/characters/e85e3e7c-ab09-479d-924a-e81575526681?systemId=ordem'] }))
  await router.load()
  expect(router.state.matches.at(-1)?.params).toMatchObject({ characterId: 'e85e3e7c-ab09-479d-924a-e81575526681' })
...
test('parâmetro malformado resulta em página não encontrada', async () => {
  const router = createAppRouter(createMemoryHistory({ initialEntries: ['/characters/invalid'] }))
