...
function isNavigationItemActive(to: (typeof navigation)[number]["to"], currentPath: string): boolean {
	return to === "/"
		? currentPath === "/"
...
function NavigationLink({ to, label, Icon, collapsed, currentPath }: (typeof navigation)[number] & { collapsed: boolean; currentPath: string }) {
	...
	return (
		<Link
			...
			className={`group flex min-h-12 items-center gap-3 rounded-2xl text-sm transition-[color,background-color,transform] duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.98] focus-visible:outline-2 focus-visible:outline-(--accent) ${collapsed ? "justify-center" : "px-3"} ${isActive ? "bg-(--accent) text-(--canvas) shadow-[0_10px_30px_color-mix(in_srgb,var(--accent)_22%,transparent)]" : "text-(--muted) hover:bg-white/5 hover:text-(--ink)"}`}
		>
	...
	return (
		<>
			<motion.aside
				...
				onFocus={onFocus}
				onBlur={onBlur}
				className="fixed inset-y-4 left-4 z-40 hidden flex-col overflow-hidden rounded-3xl border border-white/8 bg-(--surface)/96 p-2 shadow-[0_24px_80px_var(--shadow)] md:flex"
			>
				...
				<nav className="mt-6 min-h-0 flex-1 space-y-2 overflow-y-auto">
					{navigation.map((item) => <NavigationLink key={item.to} {...item} collapsed={collapsed} currentPath={currentPath} />)}
				</nav>
			...
			<nav aria-label="Navegação principal" className="fixed inset-x-3 bottom-3 z-40 grid grid-cols-5 rounded-2xl border border-white/10 bg-(--surface)/96 p-1.5 shadow-[0_20px_60px_var(--shadow)] md:hidden">
				{navigation.map(({ to, label, Icon }) => (
					<Link key={to} to={to} aria-label={label} aria-current={isNavigationItemActive(to, currentPath) ? "page" : undefined} className={`grid min-h-12 place-items-center rounded-xl focus-visible:outline-2 ${isNavigationItemActive(to, currentPath) ? "bg-(--accent) text-(--canvas)" : "text-(--muted)"}`}>
