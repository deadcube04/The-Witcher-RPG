...
const messages: Record<ErrorCode, string> = {
  INVALID_REQUEST: 'Confira os campos informados.', USER_NOT_FOUND: 'Perfil não encontrado.',
  CAMPAIGN_NOT_FOUND: 'Campanha não encontrada.', CHARACTER_NOT_FOUND: 'Ficha não encontrada.',
  RPG_SYSTEM_NOT_FOUND: 'Sistema não encontrado.', SYSTEM_MISMATCH: 'A ficha e a campanha precisam usar o mesmo sistema.',
  CONFLICT: 'Esta alteração conflita com os dados existentes.', INTERNAL_ERROR: 'Não foi possível concluir a operação.',
}
export class ApiError extends Error {
  readonly code: ErrorCode
  constructor(code: ErrorCode) { super(messages[code]); this.name = 'ApiError'; this.code = code }
}
