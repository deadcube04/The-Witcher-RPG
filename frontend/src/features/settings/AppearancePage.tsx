...
export function AppearancePage() {
  ...
  const sidebarMutation = useDomainMutation(preferencesApi.update, [keys.preferences])
  ...
  const available = systems.data.find((system) => system.id === preferences.data.activeSystemId)?.availableThemes ?? []
  const themeChoices = [defaultTheme, ...themes.filter((theme) => available.includes(theme.id))]
  return <><PageHeader eyebrow="Configurações / Aparência" title="A atmosfera da sua história" description="Escolha um elemento. Cada tema transforma a linguagem do seu arquivo." />
    ...
    <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{themeChoices.map((theme) => {
      ...
      return <section key={theme.id} className={theme.classes + ' flex flex-col bg-(--canvas) p-5 text-(--ink)'}>
          <div aria-hidden="true" className={theme.decoration + ' mb-6 flex min-h-40 flex-col justify-between p-5'}><span className="font-mono text-[10px] tracking-widest">{theme.mark}</span><span className="text-3xl">O outro lado.</span></div>
          <h3 className="text-2xl">{theme.name}</h3><p className="my-4 grow text-sm leading-6 opacity-80">{theme.description}</p>
          <RpgVisualProvider theme={theme}><RpgButton disabled={mutation.isPending || active}
            onClick={() => mutation.mutate(activeThemeId)}>{active ? theme.name + ' ativo' : 'Aplicar ' + theme.name}</RpgButton></RpgVisualProvider>
        </section>
    })}</div>
    {available.length === 0 && <div className="mt-6"><RpgEmptyState title="Sem temas específicos para este sistema"><p>Selecione Ordem Paranormal em Sistemas para explorar os cinco elementos.</p></RpgEmptyState></div>}
    <div className="mt-6"><MutationFeedback error={mutation.error} success={mutation.isSuccess} /></div></>
}
