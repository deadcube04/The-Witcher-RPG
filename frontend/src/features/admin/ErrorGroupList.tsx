...
export function ErrorGroupList({
	...
	const state = useMutation({
		mutationFn: ({ id, next }: { id: string; next: "open" | "resolved" }) =>
			errorAdminApi.setGroupState(id, next),
		onSuccess: async () => {
	...
	if (result.data.items.length === 0)
		return (
			<RpgEmptyState title="Nenhum grupo encontrado">
				Quando uma requisição falhar, o grupo aparecerá aqui.
			</RpgEmptyState>
		);
	return (
		<section className="overflow-hidden rounded-2xl border border-(--edge)/60 bg-(--surface)">
					...
					<tbody className="divide-y divide-(--edge)/50">
						{result.data.items.map((group) => (
							...
							>
								<td className="p-4">
									<span
										className={`font-mono text-xs ${group.status >= 500 ? "text-(--danger)" : "text-(--accent)"}`}
									>
										{group.status} · {group.errorCode}
									</span>
									<p className="mt-1 font-medium">
										{group.method}{" "}
										<code className="break-all text-xs">{group.route}</code>
									</p>
								...
								<td className="p-4">
									<span className="rounded-full border border-(--edge) px-3 py-1 text-xs">
										{group.state === "open" ? "Aberto" : "Resolvido"}
									</span>
								</td>
								<td className="p-4">
									<span className="font-mono">{group.availableCount}</span>
									<span className="text-(--muted)">
										{" "}
										disponíveis / {group.totalOccurrences} total
									</span>
								</td>
								<td className="p-4 text-xs text-(--muted)">
									{new Date(group.lastOccurredAt).toLocaleString()}
								</td>
								<td className="p-4">
									<RpgButton
										secondary
										...
										onClick={() =>
											state.mutate({
												id: group.id,
												next: group.state === "open" ? "resolved" : "open",
											})
										}
									>
										{group.state === "open" ? "Resolver" : "Reabrir"}
									</RpgButton>
								</td>
							</tr>
						))}
			...
			<div className="flex items-center justify-between border-t border-(--edge)/60 p-4 text-xs text-(--muted)">
				<span>
					Página {result.data.page} · {result.data.totalItems} grupos
				</span>
				<div className="flex gap-2">
					<RpgButton
						secondary
						disabled={filter.page === undefined || filter.page <= 1}
						onClick={() => onPage(Math.max(1, (filter.page ?? 1) - 1))}
					>
						Anterior
					</RpgButton>
					<RpgButton
						secondary
						disabled={
							result.data.page * result.data.pageSize >= result.data.totalItems
						}
