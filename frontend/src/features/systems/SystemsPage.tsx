	...
	return (
		<>
			<PageHeader
				...
				description="Seu sistema ativo define o contexto da aplicação. Campanhas e fichas existentes mantêm seu próprio universo."
			/>
			<div className="grid gap-5 md:grid-cols-3">
				{systems.data.map((system, index) => (
					<RpgCard key={system.id}>
						<p className="mb-12 font-mono text-xs text-(--accent)">
							UNIVERSO / 0{index + 1}
						</p>
						<h3 className="text-2xl font-semibold">{system.name}</h3>
						<p className="my-5 min-h-12 text-sm opacity-75">
							{system.description}
						</p>
						...
						<RpgButton
							secondary={preferences.data.activeSystemId !== system.id}
							disabled={
								mutation.isPending ||
								preferences.data.activeSystemId === system.id
							}
							onClick={() => mutation.mutate({ activeSystemId: system.id })}
						>
							{preferences.data.activeSystemId === system.id
								? "Sistema ativo"
