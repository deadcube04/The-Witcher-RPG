...
export function CharacterDeleteAction({
	character,
}: {
	character: CharacterSheet;
}) {
	const navigate = useNavigate();
	const mutation = useDomainMutation(characterApi.remove, [
		keys.characters,
		keys.campaigns,
	]);
	return (
		<RpgDeleteAction
			menuLabel="Mais ações da ficha"
			name={character.name}
			description="A ficha e seus dados serão excluídos. Esta ação não pode ser desfeita."
			onDelete={async () => {
				await mutation.mutateAsync(character.id);
