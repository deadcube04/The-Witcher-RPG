...
export function CharactersPage() {
	...
	if (characters.isPending || systems.isPending || campaigns.isPending)
		return <RpgSkeleton />;
	...
	const filtered = (characters.data ?? []).filter(
		(sheet) =>
			matchesName(sheet.name, filters.q) &&
			(!filters.systemId || sheet.systemId === filters.systemId) &&
			(!filters.campaignId ||
				(filters.campaignId === "standalone"
					? sheet.campaignId === null
					: filters.campaignId === "linked"
						? sheet.campaignId !== null
						: sheet.campaignId === filters.campaignId)),
	);
	return (
		<>
			<PageHeader
			...
			<CharacterFilters
				...
				onChange={update}
			/>
			<CharacterList characters={filtered} systems={systems.data ?? []} />
		</>
	);
}
