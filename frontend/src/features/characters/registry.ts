...
export type CharacterSheetDefinition = {
	...
	createData: () => CharacterInput["systemData"];
	Editor?: ComponentType<SheetEditorProps>;
	View?: ComponentType<SheetViewProps>;
};
const definitions: CharacterSheetDefinition[] = [
	{
		slug: "ordem-paranormal",
		sections: ["Identidade", "Atributos", "Recursos"],
		fields: [
			"nex",
			"classId",
			"originId",
		...
		schema: ordemDataSchema,
		createData: createOrdemData,
		Editor: OrdemEditor,
		View: OrdemView,
	},
	{
	...
	},
	{
		...
		sections: ["Identidade"],
		fields: [],
		schema: z.strictObject({ kind: z.literal("witcher") }),
		createData: () => ({ kind: "witcher" }),
	},
];
export const characterSheetRegistry = {
	get: (slug: string) =>
		definitions.find((definition) => definition.slug === slug),
};
