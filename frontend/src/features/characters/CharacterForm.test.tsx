...
import { createSeed, dndId, ordemId, systems } from "../../mocks/seed/seed";
...
test("oferece apenas campanhas compatíveis e salva ficha standalone", async () => {
	...
	renderFeature(
		<CharacterForm
			initial={{ ...createOrdemInput(ordemId), name: "Agente" }}
			systems={systems}
			campaigns={[
				...seed.campaigns,
				{
					...seed.campaigns[0],
					id: "58303937-df5a-449b-9085-606c547b9a91",
					name: "Campanha incompatível",
					systemId: dndId,
				},
			]}
			pending={false}
	...
	expect(
		screen.queryByRole("option", { name: "Campanha incompatível" }),
	...
	expect(save).toHaveBeenCalledWith(
		expect.objectContaining({
			systemData: expect.objectContaining({
				attributes: expect.objectContaining({ strength: 3 }),
				resources: expect.objectContaining({
					health: expect.objectContaining({ maximum: 20 }),
				}),
			}),
		}),
	);
