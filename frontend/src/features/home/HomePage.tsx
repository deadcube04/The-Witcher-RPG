...
export function HomePage() {
	...
	const destination =
		resumable && recent ? `/${recent.kind}/${recent.id}` : "/campaigns/new";
		...
		<div className="space-y-12 lg:space-y-16">
			<header className="flex flex-wrap items-end justify-between gap-5">
				<div>
					...
					<h1 className="mt-3 font-serif text-4xl leading-none tracking-[-0.04em] md:text-6xl">
						Suas histórias se encontram aqui.
					</h1>
				</div>
				<p className="max-w-sm text-sm leading-6 text-(--muted)">
					Suas campanhas e personagens, em um só lugar.
				</p>
			</header>

			<div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_20rem]">
				<MediaFrame
					src="/brand/nexus-celestial.svg"
					alt=""
					tone="brand"
					priority
				...
				>
					<div className="flex min-h-[32rem] max-w-3xl flex-col justify-between p-6 md:p-10 lg:p-14">
						<div className="flex items-center gap-3">
							<ArchiveEyebrow>
								{resumable ? "Último acesso" : "Sua próxima história"}
							</ArchiveEyebrow>
						</div>
						<div>
							<h2 className="max-w-2xl font-serif text-4xl leading-[0.98] tracking-[-0.04em] text-(--ink) md:text-6xl">
								{resumable && recent
									? recent.name
									: "Toda história começa com uma conexão."}
							</h2>
							<p className="mt-5 max-w-xl text-sm leading-7 text-(--muted)">
								{resumable
									? "Continue de onde parou, com sua campanha e seus personagens por perto."
									: "Crie uma campanha e reúna os personagens da sua próxima aventura."}
							</p>
							<Link
								to={destination}
								className="group mt-8 inline-flex min-h-13 items-center gap-5 rounded-full bg-(--accent) px-6 font-semibold text-(--on-accent) focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-(--accent)"
							>
								{resumable ? "Retomar sessão" : "Criar campanha"}
				...
				>
					<div className="flex items-center gap-3 text-(--accent)">
						<PiSparkleThin aria-hidden="true" className="size-5" />
						<ArchiveEyebrow>Ações rápidas</ArchiveEyebrow>
					</div>
					<div className="mt-8 flex-1 divide-y divide-(--edge)/60">
						{[
							...
							<Link
								key={to}
								to={to}
								className="group flex min-h-20 items-center justify-between gap-4 py-4 text-lg hover:text-(--accent) focus-visible:outline-2"
							>
								<span>{label}</span>
								<PiPlusThin aria-hidden="true" className="size-5" />
							</Link>
						))}
					</div>
					<Link
						to="/systems"
						className="mt-6 text-sm text-(--muted) underline decoration-(--edge) underline-offset-4 hover:text-(--accent)"
					>
						Trocar sistema ativo
					</Link>
				</ArchivePanel>
			</div>
			<div className="grid gap-10 xl:grid-cols-[minmax(0,1.3fr)_minmax(20rem,0.7fr)]">
