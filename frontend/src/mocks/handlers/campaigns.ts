	...
	return [
		http.get("*/api/v1/campaigns", () =>
			safe(() =>
				HttpResponse.json(
					repo
						.read()
						.campaigns.filter((item) => item.ownerId === repo.read().user.id),
				),
			),
		),
		...
		http.post("*/api/v1/campaigns", ({ request }) =>
			safe(async () => {
				...
				if (!state.systems.some((system) => system.id === input.systemId))
		...
		http.patch("*/api/v1/campaigns/:id", ({ request, params }) =>
			safe(async () => {
				...
				if (!state.systems.some((system) => system.id === campaign.systemId))
					fail("RPG_SYSTEM_NOT_FOUND");
				if (
					state.characters.some(
						(sheet) =>
							sheet.campaignId === campaign.id &&
							sheet.systemId !== campaign.systemId,
					)
				)
		...
		http.delete("*/api/v1/campaigns/:id", ({ params }) =>
			safe(() => {
				...
				repo.update((data) => ({
					...data,
					campaigns: data.campaigns.filter((item) => item.id !== campaign.id),
