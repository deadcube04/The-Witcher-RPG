...
import { dndId, ordemId } from "../seed/seed";
...
test("CRUD de campanha preserva fichas como standalone ao excluir", async () => {
	...
	const sheet = await characterApi.create({
		...createOrdemInput(ordemId, campaign.id),
		name: "Agente",
	});
	...
	await expect(campaignApi.get(campaign.id)).rejects.toMatchObject({
...
test("bloqueia campanha e ficha de sistemas incompatíveis", async () => {
	...
	).rejects.toMatchObject({ code: "SYSTEM_MISMATCH" });
});
...
test("CRUD de ficha standalone persiste todos os dados", async () => {
	const sheet = await characterApi.create({
		...createOrdemInput(ordemId),
		name: "Clara",
	});
	await characterApi.update(sheet.id, { background: "Arquivo recuperado" });
	expect(await characterApi.get(sheet.id)).toMatchObject({
		campaignId: null,
		background: "Arquivo recuperado",
	});
	await characterApi.remove(sheet.id);
	await expect(characterApi.get(sheet.id)).rejects.toMatchObject({
