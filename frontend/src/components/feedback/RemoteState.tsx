...
export function RpgSkeleton() {
	return (
		...
		>
			...
			<div className="grid gap-5 pt-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(18rem,0.6fr)]">
				<div className="min-h-72 animate-pulse rounded-3xl bg-(--surface-raised) motion-reduce:animate-none" />
				<div className="min-h-72 animate-pulse rounded-3xl border border-(--edge) bg-(--surface) motion-reduce:animate-none" />
			</div>
		</div>
	);
}
export function RpgErrorState({
	...
	return (
		<section
			role="alert"
			className="grid min-h-80 place-items-center rounded-3xl border border-(--edge)/70 bg-(--surface) p-8 text-center"
		>
			<div className="max-w-lg">
				<PiWarningCircleThin
					aria-hidden="true"
					className="mx-auto size-12 text-(--danger)"
				/>
				<p className="mt-6 font-mono text-[10px] uppercase tracking-[0.22em] text-(--danger)">
					Falha de conexão
				</p>
				<h2 className="mt-3 font-serif text-4xl">Não foi possível carregar</h2>
...
export function MutationFeedback({
	...
	if (error)
		return (
			<p role="alert" className="border-l-2 border-(--danger) pl-3">
				{error.message}
			</p>
		);
	return success ? (
