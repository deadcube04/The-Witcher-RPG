	...
	return (
		<div className="pointer-events-none absolute inset-0 z-30 overflow-visible">
			{active ? (
				<div
					aria-hidden="true"
					className="absolute inset-y-0 right-0 grid w-[clamp(72px,7vw,112px)] grid-rows-[repeat(4,minmax(0,1fr))] gap-4"
				>
					{items.map((item) => (
						<div
							key={item.key}
							className="grid place-items-center rounded-l-3xl border border-r-0 border-(--edge) bg-(--panel) text-3xl text-(--ink) md:text-4xl"
						>
							{item.icon}
						</div>
					))}
			...
			<AnimatePresence>
				{active && (
					<motion.div
						...
						initial={
							reduced ? false : { x: "calc(100% - clamp(72px, 7vw, 112px))" }
						}
						animate={{ x: 0 }}
						exit={
							reduced
								? undefined
								: { x: "calc(100% - clamp(72px, 7vw, 112px))" }
						}
					...
					>
						<button
							type="button"
							aria-label={`Fechar ${active.label}`}
							aria-expanded="true"
							onClick={() => setActiveKey(null)}
							className={
								"col-start-1 grid h-full w-full place-items-center rounded-l-3xl border border-r-0 border-(--accent) bg-(--panel) text-3xl text-(--accent) shadow-xl focus-visible:outline-2 focus-visible:outline-(--accent) md:text-4xl " +
								(rowClasses[activeIndex] ?? "row-start-1")
							}
						>
							{active.icon}
						...
						<section className="col-start-2 row-start-1 row-end-5 flex min-w-0 flex-col overflow-y-auto rounded-r-3xl border border-(--edge) bg-(--panel) p-5 shadow-2xl md:p-8">
							<div className="mb-6 flex items-center justify-between border-b border-(--edge) pb-4">
								...
								<button
									...
									aria-label={`Fechar ${active.label}`}
									onClick={() => setActiveKey(null)}
									className="grid size-10 place-items-center rounded-full border border-(--edge) text-xl hover:text-(--accent) focus-visible:outline-2 focus-visible:outline-(--accent)"
								>
									×
								</button>
							</div>
