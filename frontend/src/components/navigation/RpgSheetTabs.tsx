	...
	return (
		<Tabs
			...
			items={items.map(({ key, label, icon, children }) => ({
				...
				children: (
					<section className="min-h-[480px] rounded-l-2xl border border-(--edge) bg-(--panel) p-4 text-(--ink) md:min-h-[720px] md:p-6 xl:min-h-[1000px]">
						<h3 className="mb-6 border-b border-(--edge) pb-4 font-sans text-xl font-semibold">
							{label}
						</h3>
						{children}
					</section>
