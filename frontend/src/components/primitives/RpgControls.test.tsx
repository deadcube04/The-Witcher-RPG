...
function ResourceHarness({ tone }: { tone: 'health' | 'effort' | 'sanity' }) {
  ...
  return <RpgResourceBar label="Recurso" tone={tone} {...value} onChange={setValue} editable />
}

test.each(['health', 'effort', 'sanity'] as const)('barra %s funciona por teclado e limita os passos aos extremos', async (tone) => {
  render(<ResourceHarness tone={tone} />)
  const user = userEvent.setup()
  await user.tab()
  await user.keyboard('{Enter}')
  expect(screen.getByLabelText('Recurso atual')).toHaveDisplayValue('1')
  await user.click(screen.getByRole('button', { name: 'Recurso: diminuir 5' }))
  expect(screen.getByLabelText('Recurso atual')).toHaveDisplayValue('0')
  expect(screen.getByRole('button', { name: 'Recurso: diminuir 1' })).toBeDisabled()
  await user.click(screen.getByRole('button', { name: 'Recurso: aumentar 5' }))
  await user.click(screen.getByRole('button', { name: 'Recurso: aumentar 5' }))
  expect(screen.getByRole('progressbar')).toHaveAttribute('aria-valuenow', '10')
  expect(screen.getByRole('button', { name: 'Recurso: aumentar 1' })).toBeDisabled()
})
