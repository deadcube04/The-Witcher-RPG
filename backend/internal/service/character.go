...
var ErrSystemMismatch = errors.New("system mismatch")

type Character struct{ store *repository.Store }

func NewCharacter(store *repository.Store) *Character { return &Character{store: store} }

func (s *Character) List(ctx context.Context) ([]domain.Character, error) {
	ids, err := s.store.CharacterIDs(ctx)
	if err != nil {
		return nil, err
	}
	out := make([]domain.Character, 0, len(ids))
	for _, id := range ids {
		value, err := s.store.Character(ctx, id)
		if err != nil {
			return nil, err
		}
...
func (s *Character) Get(ctx context.Context, id string) (domain.Character, error) {
	return s.store.Character(ctx, id)
}
func (s *Character) Options(ctx context.Context) (domain.CharacterOptions, error) {
	...
	for _, system := range systems {
		if system.Slug == "ordem-paranormal" {
			return s.store.CharacterOptions(ctx, system.ID)
		}
	}
	return domain.CharacterOptions{}, repository.ErrNotFound
}
...
func (s *Character) Update(ctx context.Context, id string, in domain.CharacterInput) (domain.Character, error) {
	...
	return s.store.UpdateCharacter(ctx, id, in)
}
func (s *Character) Delete(ctx context.Context, id string) error {
	return s.store.DeleteCharacter(ctx, id)
}
...
func (s *Character) prepare(ctx context.Context, in *domain.CharacterInput, old *domain.Character) error {
	...
	if in.CampaignID != nil {
		ok, err := s.store.CampaignMatches(ctx, *in.CampaignID, in.SystemID)
		if err != nil {
			return err
		}
		if !ok {
	...
	rule, err := s.store.ClassRule(ctx, d.ClassID, in.SystemID)
	if err != nil {
		return err
	}
	limit, err := s.store.NEXLimit(ctx, d.NEX)
	if err != nil {
		return ErrInvalid
	}
	d.PELimit = limit
	if d.OriginID != nil {
		ok, err := s.store.OriginExists(ctx, *d.OriginID, in.SystemID)
		if err != nil {
			return err
		}
		if !ok {
