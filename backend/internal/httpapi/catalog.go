...
func (a *API) inventoryCatalog(c *gin.Context) {
	...
	value, err := service.NewCatalog(a.store).Inventory(c.Request.Context(), query, kind)
	if err != nil {
...
func (a *API) ritualCatalog(c *gin.Context) {
	...
	value, err := service.NewCatalog(a.store).Rituals(c.Request.Context(), query, element)
	if err != nil {
		a.failure(c, err, "CONTENT_NOT_FOUND")
		return
	}
...
func (a *API) attackCatalog(c *gin.Context) {
	...
	value, err := service.NewCatalog(a.store).Attacks(c.Request.Context(), query, source)
	if err != nil {
		a.failure(c, err, "CONTENT_NOT_FOUND")
		return
	}
