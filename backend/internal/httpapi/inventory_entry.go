...
func (a *API) listInventory(c *gin.Context) {
	...
	v, err := service.NewInventoryEntries(a.store).List(c.Request.Context(), id)
	if err != nil {
...
func (a *API) addInventory(c *gin.Context) {
	...
	v, err := service.NewInventoryEntries(a.store).Add(c.Request.Context(), id, in.DefinitionID, in.Quantity)
	if err != nil {
...
func (a *API) updateInventory(c *gin.Context) {
	...
	v, err := service.NewInventoryEntries(a.store).Update(c.Request.Context(), id, entryID, fields)
	if err != nil {
		a.failure(c, err, "CONTENT_NOT_FOUND")
		return
	}
...
func (a *API) deleteInventory(c *gin.Context) {
	...
	err := service.NewInventoryEntries(a.store).Delete(c.Request.Context(), id, entryID, c.Query("attackPolicy"))
	if err != nil {
		a.failure(c, err, "CONTENT_NOT_FOUND")
		return
	}
