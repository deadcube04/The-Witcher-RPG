...
func (s *Store) RitualEntries(ctx context.Context, characterID string) ([]CharacterRitual, error) {
...
func (s *Store) RitualEntry(ctx context.Context, characterID, entryID string) (RitualEntry, error) {
	...
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return RitualEntry{}, ErrNotFound
	}
	if err != nil {
		return RitualEntry{}, err
	}
...
func (s *Store) AddRitual(ctx context.Context, characterID, definitionID string) (RitualEntry, error) {
	c, err := s.Character(ctx, characterID)
	if err != nil {
		return RitualEntry{}, err
	}
	...
	if count > 0 {
		return RitualEntry{}, ErrAlreadyAdded
	}
	var id string
...
func (s *Store) UpdateRitualEntry(ctx context.Context, characterID, entryID string, fields map[string]any) (RitualEntry, error) {
	c, err := s.Character(ctx, characterID)
	if err != nil {
		return RitualEntry{}, err
	}
...
func (s *Store) DeleteRitualEntry(ctx context.Context, characterID, entryID string) error {
	if _, err := s.Character(ctx, characterID); err != nil {
		return err
	}
