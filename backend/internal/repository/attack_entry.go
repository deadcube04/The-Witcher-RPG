...
func (s *Store) AttackEntries(ctx context.Context, characterID string) ([]CharacterAttack, error) {
...
func (s *Store) AttackEntry(ctx context.Context, characterID, entryID string) (AttackEntry, error) {
	...
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return AttackEntry{}, ErrNotFound
	}
	row.CreatedAt = utcTime(row.CreatedAt)
	row.UpdatedAt = utcTime(row.UpdatedAt)
	return row, err
}
func (s *Store) validateAttackLink(ctx context.Context, characterID string, d AttackDefinition, inventoryID *string) error {
	if inventoryID == nil {
		return nil
	}
...
func (s *Store) AddAttack(ctx context.Context, characterID, definitionID string, inventoryID *string) (AttackEntry, error) {
	c, err := s.Character(ctx, characterID)
	if err != nil {
		return AttackEntry{}, err
	}
	...
	if count > 0 {
		return AttackEntry{}, ErrAlreadyAdded
	}
	var id string
...
func (s *Store) AddAttackFromInventory(ctx context.Context, characterID, inventoryID string) (AttackEntry, error) {
...
func (s *Store) UpdateAttackEntry(ctx context.Context, characterID, entryID string, fields map[string]any) (AttackEntry, error) {
	c, err := s.Character(ctx, characterID)
	if err != nil {
		return AttackEntry{}, err
	}
...
func (s *Store) DeleteAttackEntry(ctx context.Context, characterID, entryID string) error {
	if _, err := s.Character(ctx, characterID); err != nil {
		return err
	}
