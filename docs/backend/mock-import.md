...
Clique em **Prévia** para obter `ready`, `alreadyImported` e problemas por item. A prévia executa o processamento em uma transação revertida, sem gravar dados. Depois de revisar os itens, clique em **Aplicar**. A aplicação grava todos os itens selecionados numa transação; qualquer problema reverte o lote. O mapa `public.local_import_map` impede duplicação ao reenviar os mesmos IDs locais. O armazenamento local permanece no navegador e pode ser mantido como referência.
